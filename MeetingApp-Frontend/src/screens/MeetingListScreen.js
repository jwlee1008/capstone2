...
function MeetingCard({ meeting, onPress, onDelete }) {
  ...
  return (
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.78}>
      ...
      <View style={styles.cardRight}>
        <TouchableOpacity style={styles.deleteMeetingBtn} onPress={onDelete} activeOpacity={0.8}><Ionicons name="trash-outline" size={15} color={COLORS.error} /></TouchableOpacity>
        <View style={styles.sessionBadge}><Text style={styles.sessionBadgeText}>{meeting.sessions.length}</Text><Text style={styles.sessionBadgeLabel}>회</Text></View>
  ...
  const handleDeleteMeeting = (meeting) => {
    const runDelete = async () => {
      try {
        ...
        Alert.alert('삭제 완료', '회의를 삭제했습니다.');
      } catch (error) {
        Alert.alert('삭제 실패', error?.message || '회의를 삭제하지 못했습니다.');
      }
    };
    if (typeof window !== 'undefined' && window.confirm) {
      if (window.confirm(`${meeting.name} 회의를 삭제할까요?`)) runDelete();
      return;
    }
    Alert.alert('회의 삭제', `${meeting.name} 회의를 삭제할까요?`, [
