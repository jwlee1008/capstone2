...
export default function MeetingDetailScreen({ navigation, route }) {
  ...
  const { workspace, getMeetingById, refreshMeetingData, uploadRecordingAndTranscribe, updateSpeakerName, addCalendarTask, inviteMember, deleteRecording } = useAppContext();
  ...
  const calendarItems = useMemo(() => {
    ...
    const taskItems = (latestSession.tasks || []).map((task) => ({
      id: `task-${task.id}`,
      type: 'task',
    ...
    const eventItems = (latestSession.events || []).map((event) => ({
      id: `event-${event.id}`,
      type: 'event',
      title: event.title || '제목 없는 일정',
      meta: `${event.startAt ? formatDateTime(event.startAt) : '시간 미정'}${event.endAt ? ` ~ ${formatDateTime(event.endAt)}` : ''}`,
    }));
    ...
    <SafeAreaView style={styles.safeArea} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {latestSession ? <View style={styles.analysisSection}>
          ...
          <Card><View style={styles.analysisCardHeader}><Ionicons name="calendar-clear-outline" size={16} color={COLORS.primary} /><Text style={styles.analysisCardTitle}>캘린더 항목</Text><View style={styles.calendarCountPill}><Text style={styles.calendarCountText}>할일 {(latestSession.tasks || []).length} · 일정 {(latestSession.events || []).length}</Text></View></View>{calendarItems.length === 0 ? <View style={styles.calendarEmpty}><Ionicons name="calendar-outline" size={28} color={COLORS.border} /><Text style={styles.calendarEmptyText}>분석된 캘린더 항목이 없어요.</Text></View> : <View style={styles.calendarList}>{calendarItems.map((item) => <CalendarItem key={item.id} item={item} />)}</View>}</Card>
