...
export default function MeetingDetailScreen({ navigation, route }) {
  ...
  const { user, workspace, getMeetingById, refreshMeetingData, uploadRecordingAndTranscribe, updateSpeakerName, addCalendarTask, updateCalendarTask, inviteMember, deleteRecording } = useAppContext();
  ...
  const handleSaveSpeaker = async (speaker) => {
    try {
      await updateSpeakerName(meeting.id, latestSession.id, selectedSpeaker, speaker);
      setSelectedSpeaker(null);
      Alert.alert('저장 완료', '화자 매핑을 저장했습니다.');
    } catch (error) {
    ...
    <SafeAreaView style={styles.safeArea} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {latestSession ? <View style={styles.analysisSection}>
          <SectionTitle title="처리 상태" /><Card><View style={styles.analysisCardHeader}>{latestSession.processStatus === 'done' ? <Ionicons name="checkmark-circle-outline" size={18} color={COLORS.success} /> : <ActivityIndicator size="small" color={COLORS.warning} />}<Text style={styles.analysisCardTitle}>{latestSession.processStatus === 'done' ? '대화 내용 확인 가능' : '기록 정리중'}</Text>{latestSession.recordingId ? <TouchableOpacity style={styles.recordingDeleteBtn} onPress={handleDeleteRecording} activeOpacity={0.8}><Ionicons name="trash-outline" size={15} color={COLORS.error} /></TouchableOpacity> : null}</View><Text style={styles.cardDesc}>{latestSession.fileName || '녹음 파일'} · 화자 구분 완료</Text></Card>
