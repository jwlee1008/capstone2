    ...
    <SafeAreaView style={styles.safeArea} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.controlArea}>
          <TouchableOpacity style={[styles.uploadBtn, isRecording && styles.recordingBtn, isUploading && styles.uploadBtnDisabled]} onPress={isRecording ? handleStopRecordingAndUpload : handleStartRecording} activeOpacity={0.85} disabled={isUploading}>
            <View style={styles.uploadBtnLeft}><View style={[styles.uploadIconWrap, isRecording && styles.recordingIconWrap]}>{isUploading ? <ActivityIndicator size="small" color={COLORS.primary} /> : <Ionicons name={isRecording ? 'stop-circle' : 'mic-outline'} size={22} color={isRecording ? COLORS.error : COLORS.primary} />}</View><View style={{ flex: 1 }}><Text style={styles.uploadBtnTitle}>{isRecording ? `녹음 중 ${recordingDuration}` : (isUploading ? '업로드 및 STT 진행중' : '직접 녹음 시작')}</Text><Text style={styles.uploadBtnDesc}>{isRecording ? '누르면 녹음을 종료하고 바로 정리합니다' : '앱에서 녹음한 뒤 바로 STT를 시작합니다'}</Text></View></View><Ionicons name={isRecording ? 'checkmark' : 'chevron-forward'} size={18} color={isRecording ? COLORS.error : COLORS.border} />
          </TouchableOpacity>
          {isRecording && !isUploading ? <TouchableOpacity style={styles.recordCancelBtn} onPress={handleCancelRecording} activeOpacity={0.85}><Ionicons name="close" size={16} color={COLORS.error} /><Text style={styles.recordCancelText}>녹음 취소</Text></TouchableOpacity> : null}
          <TouchableOpacity style={[styles.uploadBtn, (isUploading || isRecording) && styles.uploadBtnDisabled]} onPress={handlePickFile} activeOpacity={0.85} disabled={isUploading || isRecording}><View style={styles.uploadBtnLeft}><View style={styles.uploadIconWrap}>{isUploading ? <ActivityIndicator size="small" color={COLORS.primary} /> : <Ionicons name="folder-open-outline" size={22} color={COLORS.primary} />}</View><View style={{ flex: 1 }}><Text style={styles.uploadBtnTitle}>{isUploading ? '업로드 및 STT 진행중' : '녹음 파일 업로드'}</Text><Text style={styles.uploadBtnDesc}>서버에 녹음 파일을 저장하고 STT를 시작합니다</Text></View></View><Ionicons name="chevron-forward" size={18} color={COLORS.border} /></TouchableOpacity>
        </View>
