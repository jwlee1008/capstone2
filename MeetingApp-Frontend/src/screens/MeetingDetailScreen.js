...
export default function MeetingDetailScreen({ navigation, route }) {
  ...
  const registerTask = async (task) => {
    if (task.id) {
      Alert.alert('이미 등록됨', '분석된 할일은 캘린더에 이미 반영되어 있습니다.');
      return;
    }
...
const styles = StyleSheet.create({
  registeredBadge: { flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: '#F0FDF4', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 4, marginLeft: 8 },
  registeredBadgeText: { fontSize: 11, fontWeight: '700', color: COLORS.success },
